<?php get_header(); ?>
<div class="htheme_content_holder [ content-guide-size ]">
	<div class="htheme_row_margin_top_bottom htheme_vc_row_contained">
		<div class="wpb_column vc_column_container vc_col-sm-12">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper"><!-- ROW -->
					<div class="htheme_row">
						<div class="htheme_container">
							<div class="htheme_inner_col"><!-- TITLE DOUBLE TOP BOTTOM -->
								<div class="htheme_title_container" data-title-type="top_bottom">
									<div class="htheme_title">
										<h2>Guía de tallas</h2>
									</div>
								</div>
							</div>
						</div>
					</div><!-- ROW -->
				</div>
			</div>
		</div>
	</div>
	<div class="htheme_vc_row_contained">
		<div class="wpb_column vc_column_container vc_col-sm-8">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<p>Lorem ipsum dolor sit amet, consectetur adipiscing, sed do eiusmod tempor incididunt ut labore et dolore magna aliquaodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
								<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, tempor incididunt ut labore et dolore magna aliquaodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliquaodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
								<p>Sed do eiusmod tempor incididunt ut labore et dolore magna aliquaodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliquaodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="wpb_column vc_column_container vc_col-sm-4">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<img src="<?php echo get_stylesheet_directory_uri(); ?>/htheme/assets/images/size.png" alt="imagen de tallas de cabeza">
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="htheme_vc_row_contained">
		<div class="wpb_column vc_column_container vc_col-sm-6">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<a href="http://localhost:8888/new-era/knits/"><h4>59FIFTY &amp; 19TWENTY</h4></a>
								<table class="[ text-center ]" border="0" cellspacing="0" cellpadding="0">
									<tr class="[ background-color--primary ]">
										<th class="[ paddin-top-bottom ]">Circunferencia (cm)</th>
										<th class="[ paddin-top-bottom ]">Talla gorra</th>
										<th class="[ paddin-top-bottom ]">Edad</th>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>48.2</td>
										<td>6</td>
										<td>Infant</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>51.1</td>
										<td>6 3/8</td>
										<td>Toddler</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>52</td>
										<td>6 1/2</td>
										<td rowspan="2">Child</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>53</td>
										<td>6 5/8</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>53.9</td>
										<td>6 3/4</td>
										<td rowspan="2">Youth</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>54.9</td>
										<td>6 7/8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>55.8</td>
										<td>7</td>
										<td rowspan="11">Adult</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>56.8</td>
										<td>7 1/8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>57.7</td>
										<td>7 1/4</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>58.7</td>
										<td>7 3/8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>59.6</td>
										<td>7 1/2</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>60.6</td>
										<td>7 5/8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>61.5</td>
										<td>7 3/4</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>62.5</td>
										<td>7 7/8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>63.5</td>
										<td>8</td>
									</tr>
								</table>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="wpb_column vc_column_container vc_col-sm-6">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<a href="http://localhost:8888/new-era/knits/"><h4>39THIRTY</h4></a>
								<table class="[ text-center ]" border="0" cellspacing="0" cellpadding="0">
									<tr class="[ background-color--primary ]">
										<th class="[ paddin-top-bottom ]">Circunferencia (cm)</th>
										<th class="[ paddin-top-bottom ]" colspan="2">Talla gorra</th>
									</tr>
									<tr>
										<td class="[ background-color--gray--light ]">53.9</td>
										<td class="[ background-color--gray--light ]">Extra Chica - Chica</td>
									</tr>
									<tr>
										<td class="[ background-color--gray--dark ]">54.9</td>
										<td class="[ background-color--gray--dark ]"></td>
										<td class="[ background-color--gray--medium ]" colspan="2" rowspan="4">Chica - Mediana</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td class="[ background-color--gray--medium ]">55.8</td>
										<td class="[ background-color--gray--medium ]" rowspan="2"></td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td class="[ background-color--gray--medium ]">56.8</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td  class="[ background-color--gray--dark ]">57.7</td>
										<td class="[ background-color--gray--dark ]"></td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>58.7</td>
										<td rowspan="2">Mediana - Grande</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>59.6</td>
									</tr>
									<tr>
										<td class="[ background-color--gray--dark ]">60.6</td>
										<td class="[ background-color--gray--dark ]"></td>
										<td class="[ background-color--gray--medium ]" colspan="2" rowspan="4">Grande - Extra grande</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>61.5</td>
										<td class="[ background-color--gray--medium ]" rowspan="3"></td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>62.5</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>63.5</td>
									</tr>
								</table>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="htheme_vc_row_contained">
		<div class="wpb_column vc_column_container vc_col-sm-6">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<a href="http://localhost:8888/new-era/knits/"><h4>9FIFTY</h4></a>
								<table class="[ text-center ]" border="0" cellspacing="0" cellpadding="0">
									<tr class="[ background-color--primary ]">
										<th class="[ paddin-top-bottom ]">Circunferencia (cm)</th>
										<th class="[ paddin-top-bottom ]" colspan="3">Talla gorra</th>
									</tr>
									<tr>
										<td class="[ background-color--gray--light ]">54.9</td>
										<td class="[ background-color--gray--light ]" rowspan="6">Chica - Mediana</td>
										<td class="[ background-color--gray--dark ]" rowspan="2"></td>
										<td class="[ background-color--gray--dark ]" rowspan="8">Unitalla</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>55.8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>56.8</td>
										<td class="[ background-color--gray--medium ]" rowspan="6">Mediana - Grande</td>
									</tr>
									<tr>
										<td class="[ background-color--gray--light ]">57.7</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>58.7</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>59.6</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>60.6</td>
										<td></td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>61.5</td>
										<td></td>
									</tr>
								</table>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="wpb_column vc_column_container vc_col-sm-3">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<a href="http://localhost:8888/new-era/knits/"><h4>9FORTY & 9TWENTY</h4></a>
								<table class="[ text-center ]" border="0" cellspacing="0" cellpadding="0">
									<tr class="[ background-color--primary ]">
										<th class="[ paddin-top-bottom ]">Circunferencia (cm)</th>
										<th class="[ paddin-top-bottom ]" colspan="3">Talla gorra</th>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>55.8</td>
										<td rowspan="9">Unitalla</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>56.8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>57.7</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>58.7</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>59.6</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>60.6</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>61.5</td>
									</tr>
								</table>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="wpb_column vc_column_container vc_col-sm-3">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
							<div class="htheme_inner_col htheme_default_content">
								<a href="http://localhost:8888/new-era/knits/"><h4>BUCKET</h4></a>
								<table class="[ text-center ]" border="0" cellspacing="0" cellpadding="0">
									<tr class="[ background-color--primary ]">
										<th class="[ paddin-top-bottom ]">Circunferencia (cm)</th>
										<th class="[ paddin-top-bottom ]" colspan="3">Talla gorra</th>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>54.9</td>
										<td rowspan="3">Extra CH o CH/M</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>55.8</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>56.8</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>57.7</td>
										<td>CH</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>58.7</td>
										<td>M O M/G</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>59.6</td>
										<td>G</td>
									</tr>
									<tr class="[ background-color--gray--light ]">
										<td>60.6</td>
										<td>Extra G o G/Extra G</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>61.5</td>
										<td rowspan="3">Extra Extra G</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>62.5</td>
									</tr>
									<tr class="[ background-color--gray--medium ]">
										<td>63.5</td>
									</tr>
								</table>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<?php get_footer(); ?>
