...
function get_ids_of_all_attachments_callback()
{
    ...
    $the_single_url = implode("' , '", $extract_all_urls);
    $get_new_urls = array();
    ...
    if (!empty($get_new_urls)) {
        echo implode(':::::', $get_new_urls);
    } else {
        echo '';
    }
...
function enqueuezoomscript()
{
    ...
    if (!is_admin()) {
        global $post;
        $zlevel = get_option('woocommerce_zoom_option', '2');
        ...
        if (defined('SCRIPT_DEBUG') && (SCRIPT_DEBUG == true)) {
            ...
            wp_register_script('original_zoom', plugins_url('/js/jquery.zoom_.min.js', __FILE__), array('jquery'), false, true);
            wp_enqueue_script('original_zoom');
            wp_register_script('bxsliderjs', plugins_url('/js/jquery.bxSlider_2.min.js', __FILE__), array('jquery'), false, true);
            wp_enqueue_script('bxsliderjs');
            wp_register_script('enqueuezoomjs', plugins_url('/js/swap_images.min.js', __FILE__), array('jquery'), false, true);
            wp_enqueue_script('enqueuezoomjs');
            wp_register_style('bxslidercss', plugins_url('/css/jquery.bxslider.min.css', __FILE__));
            wp_enqueue_style('bxslidercss');
        }
        ...
        wp_localize_script('enqueuezoomjs', 'obj', $translation_array);
...
if ((isset($_GET['page']) &&  'zoom-extension-plugin' == $_GET['page'])) {
...
function wdm_zoom_extension_styles()
{
    if (defined('SCRIPT_DEBUG') && (SCRIPT_DEBUG == true)) {
        ...
        wp_register_style('tip-tip-style', plugins_url('/css/tipTip.min.css', __FILE__));
        wp_enqueue_style('tip-tip-style');
    }
}
...
function wdm_zoom_extension_scripts()
{
    ...
    if (defined('SCRIPT_DEBUG') && (SCRIPT_DEBUG == true)) {
        ...
        wp_register_script('tip-tip-script', plugins_url('/js/jquery.tipTip.min.js', __FILE__), array('jquery'));
        wp_enqueue_script('tip-tip-script');
    }
}
...
function zoom_extension_plugin_options_page()
{
    ...
    if ($_POST && 'Submit' == $_POST['submit_zoom_level_option']  && !empty($_POST['zoom_level_option']) && (false !== filter_var($_POST['zoom_level_option'], FILTER_VALIDATE_INT))) {
	 ...
	 <p style="font-weight:bold">Settings Saved.</p>
	    ...
	    and do post an encouraging review, here - <a href="http://wordpress.org/support/view/plugin-reviews/<?php echo $wdm_plugin_slug;
        ?>" target="_blank"> Review this plugin</a>.
	</div>

	<?php

    } elseif ($_POST && 'Submit' == $_POST['submit_zoom_level_option']  && empty($_POST['zoom_level_option'])) {
        ?>
      <div class="error"><p style="font-weight:bold">Empty value can not be saved. Setting Zoom level to default value.</p></div>
	<?php

    } elseif ($_POST && 'Submit' == $_POST['submit_zoom_level_option']  && filter_var($_POST['zoom_level_option'], FILTER_VALIDATE_INT) === false) {
        ?>
	 ...
	 <div class="postbox">
	    ...
	    <div class="inside">
		  ...
		  <div id="right-side">
                ...
                $wdm_plugin_slug = 'zwoom-woocommerce-product-image-zoom-extension-by-wisdmlabs';
                include_once 'wisdm_sidebar/wisdm-sidebar.php';
...
function wdm_get_update_notifications($args)
{
    $transient_name = 'wc_upgrade_notice_'.$args['Version'];

    if (false === ($upgrade_notice = get_transient($transient_name))) {
        $response = wp_safe_remote_get('https://plugins.svn.wordpress.org/zwoom-woocommerce-product-image-zoom-extension-by-wisdmlabs/trunk/readme.txt');

        if (!is_wp_error($response) && !empty($response['body'])) {
            $upgrade_notice = wdm_parse_update_notice($response['body']);
 ...
 * @return string
 */
function wdm_parse_update_notice($content)
{
    $dir = plugin_dir_path(__FILE__);
    $path = get_plugin_data($dir.'/zwoom-woocommerce-product-image-zoom-extension-by-wisdmlabs.php', 'true', 'true');
    $zwoom_version = $path['Version'];
    // Output Upgrade Notice
    $matches = null;
    $regexp = '~==\s*Upgrade Notice\s*==\s*=\s*(.*)\s*=(.*)(=\s*'.preg_quote($zwoom_version).'\s*=|$)~Uis';
    $upgrade_notice = '';
    ...
    if (preg_match($regexp, $content, $matches)) {
        ...
        if (version_compare($zwoom_version, $version, '<')) {
