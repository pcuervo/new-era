<?php
	...
	class hutility_checkin{
		...
		public function __construct($plugin_slug,$plugin_name,$plugin_friendly_name,$api_version){
			...
			//set blog url
			$this->blog_url = $hutility_helper->get_blog_domain();
			//get plugin info
		...
		private function get_plugin_info(){ //extract info from db
			...
			if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $wpdb->base_prefix ."hplugin_root")) == $wpdb->base_prefix .'hplugin_root'){
				$plugin_lookup = $wpdb->get_results($wpdb->prepare("SELECT `plugin_version`, `plugin_uuid`, DATE(`date_created`) AS 'installed' FROM `". $wpdb->base_prefix ."hplugin_root` WHERE `plugin_name` = %s;", $this->plugin_name));
				if($plugin_lookup){
